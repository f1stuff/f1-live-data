...
def add_driver_color_to_dashboard_overrides(path_to_dashboard_json: str, dashboard_titles_to_process: List[str]):
    ...
    for driver_info in D_LOOKUP:
        driver_color_list_overrides.append(_color_matcher_section(driver_info[1], driver_info[3]))
    ...
    for panel in parsed_json['panels']:
        if panel['title'] in dashboard_titles_to_process and 'fieldConfig' in panel and 'overrides' in panel['fieldConfig']:
            print("==================", panel['title'], "==================")
...
def _color_matcher_section(driver_name, driver_color_hex):
    return {
        "matcher": {
            "id": "byName",
            "options": driver_name
